...
public class CursoDaoCallableStatement implements CursoDao {
    ...
    @Override
    public List<Curso> findByCreditos(int min, int max) {return List.of();}

    @Override
    public List<Curso> findByNombre(String nombre) {return List.of();}
    ...
    public Curso find(String id) {
        ...
        try(Connection con = DBConn.getConnection();
            ...
            rs = cst.executeQuery();
            if (rs.next()) {
                curso = new Curso(rs.getString("chrCurCodigo"), rs.getString("vchCurNombre"), rs.getInt("intCurCreditos"));
            }
        }catch (SQLException e){
    ...
    public List<Curso> findAll() {
        ...
        try (Connection con = DBConn.getConnection();
            ...
            rs = cst.executeQuery();
            while (rs.next()) {
                cursos.add(new Curso(rs.getString("chrCurCodigo"), rs.getString("vchCurNombre"), rs.getInt("intCurCreditos")));
            }
        } catch (SQLException e) {
            System.out.println("Error en la consulta");
        }
